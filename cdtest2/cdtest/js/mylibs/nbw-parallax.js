...
if (screen.width > 1024) {
	...
	$(document).ready(function() { //when the document is ready...
		...
		var $BG35a 	= $('#star-crossed2');				//Sam Weber
		var $BG35b 	= $('#star-crossed2').find('.b');	//
		var $BG35c 	= $('#star-crossed2').find('.frame');//frame
		var $BG36a 	= $('#star-crossed3');				//Sam Weber
		var $BG36b 	= $('#star-crossed3').find('.b');	//
		var $BG36c 	= $('#star-crossed3').find('.frame');//frame
		var $BG37a 	= $('#star-crossed4');				//Sam Weber
		var $BG37b 	= $('#star-crossed4').find('.b');	//
		var $BG37c 	= $('#star-crossed4').find('.frame');//frame
		var $BG38a 	= $('#star-crossed5');				//Sam Weber
		var $BG38b 	= $('#star-crossed5').find('.b');	//
		var $BG38c 	= $('#star-crossed5').find('.frame');//frame
	    var $BG39a 	= $('#time');				//Sam Weber
		var $BG39b 	= $('#time').find('.b');	//
		var $BG39c 	= $('#time').find('.frame');//frame
		var $BG40a 	= $('#donate');				//Sam Weber
		var $BG40b 	= $('#donate').find('.b');	//
		var $BG40c 	= $('#donate').find('.frame');//frame
		...
		*/
		...
		if($BG3a.hasClass("inview")){ // Intro Her - Chris Neal
			...
			$BG3b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 2),  .4, 0), 'height': panelHeight});
			$BG3c.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 3),  -.2, 0), 'height': panelHeight});
			$BG3d.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 3.2), -.4, 0), 'height': panelHeight}); 
		...
		if($BG5a.hasClass("inview")){ // Intro Him - Frank Stockton
			$BG5a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 4), .3, 0)}); 
			$BG5b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 3.6), .45, 0), 'height': panelHeight}); 
			$BG5c.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 3.6),  .6, 0), 'height': panelHeight}); 
			...
			if($BG7a.hasClass("inview")){ // Discovery - Nick Felton
				$BG7a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 4),  .2, 0)});
				$BG7b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 4), -.2, 0), 'height': panelHeight});
			...
			if($BG9a.hasClass("inview")){ // Star Crossed - producer
				$BG9a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 5.8), .6, 0)}); //jessica
				$BG9b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 5.8), -2, 0), 'height': panelHeight});
		...
		if($BG35a.hasClass("inview")){ // Star Crossed 2 - Sam Weber 
				$BG35a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 6.9), .6, 0)}); //jessica
				$BG35b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 6.9), -2, 0), 'height': panelHeight});
				$BG35c.css({'height': panelHeight}); // frame
				if (pos >= ((panelHeight * 7) + (panelHeight / 2))) {
					mainPanel = $BG9a;
					ChangeLinks();
				}
	...
	if($BG36a.hasClass("inview")){ // Star Crossed 3 - Sam Weber 
				$BG36a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 8), .6, 0)}); //jessica
				$BG36b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 8), -2, 0), 'height': panelHeight});
				$BG36c.css({'height': panelHeight}); // frame
				if (pos >= ((panelHeight * 7) + (panelHeight / 2))) {
					mainPanel = $BG36a;
					ChangeLinks();
				}
...
if($BG37a.hasClass("inview")){ // Star Crossed 4 - Sam Weber 
				$BG37a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 9.2), .6, 0)}); //jessica
				$BG37b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 9.2), -2, 0), 'height': panelHeight});
				$BG37c.css({'height': panelHeight}); // frame
				if (pos >= ((panelHeight * 7) + (panelHeight / 2))) {
					mainPanel = $BG37a;
					ChangeLinks();
				}
...
if($BG38a.hasClass("inview")){ // Star Crossed 5 - Sam Weber 
				$BG38a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 10.4), .6, 0)}); //jessica
				$BG38b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 10.4), -2, 0), 'height': panelHeight});
				$BG38c.css({'height': panelHeight}); // frame
				if (pos >= ((panelHeight * 7) + (panelHeight / 2))) {
					mainPanel = $BG38a;
					ChangeLinks();
				}
...
if($BG39a.hasClass("inview")){ // timeline - Sam Weber 
				$BG39a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 11.4), .6, 0)}); //jessica
				$BG39b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 11.4), -2, 0), 'height': panelHeight});
				$BG39c.css({'height': panelHeight}); // frame
				if (pos >= ((panelHeight * 7) + (panelHeight / 2))) {
					mainPanel = $BG39a;
					ChangeLinks();
				}
...
if($BG40a.hasClass("inview")){ // donate- Sam Weber 
				$BG40a.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 12), .6, 0)}); //jessica
				$BG40b.css({'backgroundPosition': CalculateShift('middle', 'static', pos, (panelHeight * 12), -2, 0), 'height': panelHeight});
				$BG40c.css({'height': panelHeight}); // frame
				if (pos >= ((panelHeight * 7) + (panelHeight / 2))) {
					mainPanel = $BG40a;
					ChangeLinks();
				}
